...
pub static IMAGE_A_HANDLE: LazyLock<image::Handle> =
    LazyLock::new(|| image::Handle::from_bytes(IMAGE_A));
pub static IMAGE_B_HANDLE: LazyLock<image::Handle> =
    LazyLock::new(|| image::Handle::from_bytes(IMAGE_B));
pub static ACCOUNT_DEFAULT_HANDLE: LazyLock<image::Handle> =
...
impl Lock {
    ...
    pub fn view(&self, _window: Id) -> Element<Message> {
        let Lock { steps, .. } = self;
...
impl AuthSteps {
    ...
    pub fn update(&mut self, msg: StepMessage) -> Command<Message> {
...
impl<'a> AuthStep {
    ...
    pub fn welcome(user_name: &str, user_icon: image::Handle) -> Element<StepMessage> {
